...
interface OngoingWorkOrdersResponse {
  workOrders: WorkOrder[];
  total: number;
  page: number;
  totalPages: number;
}
export default function OngoingWorkOrdersPage() {
  ...
  const fetchWorkOrders = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiClient.get<OngoingWorkOrdersResponse>(`/work-orders/ongoing?${queryParams}`);
      if (response.success && response.data) {
        ...
        toast.showError('Error', response.error || 'Failed to fetch ongoing work orders');
      }
    } catch {
      toast.showError('Error', 'An error occurred while fetching ongoing work orders');
    } finally {
      setLoading(false);
    }
  }, [filters, toast]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Ongoing Work Orders</h1>
            <p className="text-gray-600">Monitor and manage active tasks in progress</p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
          <div className="flex items-center space-x-4">
            <p className="text-gray-600">
              Showing <span className="font-medium">{filteredWorkOrders.length}</span> of <span className="font-medium">{total}</span> ongoing work orders
            </p>
            {searchQuery && (
      ...
      {filteredWorkOrders.length === 0 && !loading ? (
        <Card className="text-center py-12">
          <div className="text-gray-500">
            ...
            <p className="text-lg font-medium mb-2">No ongoing work orders found</p>
        ...
        <div className="space-y-3">
          {filteredWorkOrders.map((workOrder) => (
            <Card key={workOrder.id} className="hover:shadow-md transition-all duration-200 border-l-4 border-l-blue-500">
              <div className="p-6">
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className={`px-3 py-1 text-xs font-medium rounded-full ${getStatusBadge(workOrder.status)}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-gray-400 text-lg">⏰</span>
                        <div>
                          <p className="text-xs text-gray-500 uppercase tracking-wide">Allocation Time</p>
                          <p className="text-sm font-medium text-gray-900">{formatDate(workOrder.job_allocation_time)}</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-gray-400 text-lg">📊</span>
                  ...
                  <div className="flex flex-col space-y-2 lg:ml-6">
                    ...
                    {workOrder.status === 'ongoing' && (
                      <Button
                        size="sm"
                        onClick={() => router.push(`/work-orders/${workOrder.id}`)}
                        className="bg-green-600 hover:bg-green-700 text-white flex items-center space-x-2 w-full lg:w-auto justify-center"
                      >
                        <span>✅</span>
