...
import { apiClient, getApiBaseUrl } from '@/app/utils/api';
...
export default function ReportsPage() {
  ...
  const fetchWorkOrders = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast.showError('Error', response.error || 'Failed to fetch work orders for reports');
      }
    } catch {
      toast.showError('Error', 'An error occurred while fetching work orders for reports');
    } finally {
      setLoading(false);
    }
  }, [filters, toast]);
  ...
  const generateReport = async (workOrderId: number) => {
    try {
      const response = await fetch(`${getApiBaseUrl()}/reports/work-order-sheet?workOrderId=${workOrderId}`, {
        headers: {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
          <div>
            ...
            <p className="text-gray-600">Generate comprehensive reports for completed work orders</p>
            {user && (user.role === 'user') && (
              <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <span className="font-medium">📊 Report Access:</span> You can generate reports for work orders once their completion has been approved by an administrator.
                </p>
              </div>
            )}
          </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <div className="relative">
          ...
          <Input
            type="text"
            placeholder="Search by work order number, equipment, work type, or requester..."
            value={searchQuery}
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className={`px-3 py-1 text-xs font-medium rounded-full ${
                          ...
                          {workOrder.status === 'completion_requested' ? 'Completion Requested' : workOrder.status}
                        </span>
                      </div>
                    </div>
                    {}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-gray-400 text-lg">📅</span>
                        <div>
                          <p className="text-xs text-gray-500 uppercase tracking-wide">Completed Date</p>
                    ...
                    {workOrder.status === 'completed' && (
                      <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                        <div className="flex items-center justify-between">
                          ...
                          {!canGenerateReport(workOrder) && user && (user.role === 'user') && (
                            <span className="text-xs text-gray-500">
                              Report available after approval
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="flex flex-col space-y-2 lg:ml-6">
                    ...
                    {canGenerateReport(workOrder) && (
                      <Button
                        ...
                        className="bg-blue-600 hover:bg-blue-700 text-white flex items-center space-x-2 w-full lg:w-auto justify-center"
                      >
                        <span>📥</span>
